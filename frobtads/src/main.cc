...
int main( int argc, char** argv )
{
	...
	// A semi-functional, portable ostream is defined in options.h.
	ostream cerr(stderr);
	ostream cout(stdout);
	...
	switch (vm_get_game_type(filename, actualFilename, OSFNMAX, defExts, 2)) {
	  case VM_GGT_TADS2:
		...
		return FrobTadsApplication(frobOpts).runTads(actualFilename, 0);

	  case VM_GGT_TADS3: {
		// It's Tads 3.
		int t3vmRet = FrobTadsApplication(frobOpts).runTads(actualFilename, 1);
